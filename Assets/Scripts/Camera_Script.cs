...
public class Camera_Script: MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (!target)
        {
            Debug.LogError("T$$anonymous$$s camera has no target, you need to assign a target in the inspector.");
            return;
        }

        if (doZoom)
        {
            // Record our mouse input.  If we zoom add t$$anonymous$$s to our height and distance.
